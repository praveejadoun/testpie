...
//echo '<div style="float:left; width:300px;"><span style="font-size:14px; font-weight:bold; color:#666;">Subject</span>&nbsp;&nbsp;'; 
        ...
        <?php $student = Students::model()->findAll('is_deleted=:x AND id=:y', array(':x' => 0, ':y' => $_REQUEST['stud'])); ?>
                    <td style="padding:8px 0px;"><?php foreach ($student as $student_1) {
            echo $student_1->admission_no; ?></td>
                        <td style="padding:8px 0px;"><?php echo $student_1->first_name;
        } ?></td>
        <?php ?>
        <?php $bat = Batches::model()->findAll('is_deleted=:x AND id=:y', array(':x' => 0, ':y' => $student_1->batch_id)); ?>
                    <td style="padding:8px 0px;"><?php foreach ($bat as $batch) {
            echo $batch->name;
        } ?></td>
                    <?php $cou = Courses::model()->findAll('is_deleted=:x AND id=:y', array(':x' => 0, ':y' => $batch->course_id)); ?>
                    <td style="padding:8px 0px;"><?php foreach ($cou as $course) {
        ...
        <?php
        foreach ($std as $std_1) {
            $eg = ExamGroups::model()->findAll('batch_id=:x', array(':x' => $std_1->batch_id));
        }
        ?>
        <?php
        foreach ($eg as $eg_1) {
            $ex = Exams::model()->findAll('exam_group_id=:x', array(':x' => $eg_1->id));
            ?>
            ...
            <?php foreach ($ex as $ex_1) {
                ...
                ?>
                        <tr>
                            <td style="padding:8px 0px;"><?php echo $ex_1->subject_id ?></td>
                            <td style="padding:8px 0px;">123</td>

                            <td style="padding:8px 0px;"></td>

                            <td style="padding:8px 0px;">ddd</td>
                        </tr>
            ...
            <?php $Examscore = ExamScores::model()->findAll('student_id=:x ', array(':x' => $_REQUEST['stud'])) ?>
                        ...
                        foreach ($Examscore as $Examscore_1) {
                            ...
                            foreach ($exam as $exam_1) {
                                $subject = Subjects::model()->findAll('id=:x', array(':x' => $exam_1->subject_id));
                                ?>

                                <td style="padding:8px 0px;"><?php foreach ($subject as $subject_1) {
...
//$ex= Exams::model()->findAll('exam_group_id=:x',array(':x'=>$eg_1->id));foreach($ex as $ex_1){echo $ex_1->id;} 
        ?>
            <!--<h4 style="margin-left: 16px;">Exam Name :<?php echo $eg_1->name; ?></h4>
             <div class="tablebx" style="margin:18px;"> 
                    <table width="100%" cellspacing="0p" cellpadding="0" border="0">
                    ...
                    <tr>
        <?php $Examscore = ExamScores::model()->findAll('student_id=:x ', array(':x' => $_REQUEST['stud'])) ?>
        <?php foreach ($Examscore as $Examscore_1) {
            $exam = Exams::model()->findAll('id=:x', array(':x' => $Examscore_1->exam_id));
            ?>
            <?php foreach ($exam as $exam_1) {
                $subject = Subjects::model()->findAll('id=:x', array(':x' => $exam_1->subject_id));
                ?>
