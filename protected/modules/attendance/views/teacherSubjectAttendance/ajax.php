...
<?php
$find = EmployeeSubjectwiseAttendances::model()->findAll("attendance_date=:x AND employee_id=:y AND class_timing_id=:z AND weekday_id=:w AND batch_id=:v", array(':x'=>$year.'-'.$month.'-'.$day,':y'=>$emp_id,':z'=>$c_t_id,'w'=>$weekday_id,'v'=>$batch_id));
if(count($find)==0)
{
...
echo CHtml::ajaxLink(Yii::t('job','mark leave'),$this->createUrl('teachersubjectattendance/Addnew'),array(
        ...
        'update'=>'#jobDialog','type' =>'GET','data'=>array('day' =>$day,'month'=>$month,'year'=>$year,'emp_id'=>$emp_id,'batch_id'=>$batch_id,'c_t_id'=>$c_t_id,'weekday_id'=>$weekday_id),
