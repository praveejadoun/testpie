...
class StudentDocumentController extends RController {
    ...
    public function actionDownloadImage1() {
        ...
        Yii::app()
                ->request
                ->sendFile(
                        $model->document_file_name, file_get_contents($fileDir . $model->document_file_name), $model->document_data
        );
    }
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['StudentDocument'])) {
            ...
            if ($file = CUploadedFile::getInstance($model, 'document_data')) {
                ...
                move_uploaded_file($file->tempName, 'uploadedfiles/student_document/' . $file->name);
            }
            if ($model->save())
